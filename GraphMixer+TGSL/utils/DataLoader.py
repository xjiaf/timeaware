...
def get_link_prediction_data(dataset_name: str, val_ratio: float, test_ratio: float):
    ...
    edge_raw_features = np.load('./processed/ml_{}.npy'.format(dataset_name, dataset_name))
    node_raw_features = np.load('./processed/ml_{}_node.npy'.format(dataset_name, dataset_name))
    ...
    src_node_ids = graph_df.u.values.astype(int)
    dst_node_ids = graph_df.i.values.astype(int)
    node_interact_times = graph_df.ts.values.astype(np.float64)
    edge_ids = graph_df.idx.values.astype(int)
